...
// .wrangler/tmp/pages-fPfxAe/functionsWorker-0.31663766019398165.mjs
var __defProp2 = Object.defineProperty;
var __name2 = /* @__PURE__ */ __name((target, value) => __defProp2(target, "name", { value, configurable: true }), "__name");
var inMemoryRateLimitState = /* @__PURE__ */ new Map();
var EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
var sanitizeValue = /* @__PURE__ */ __name2((value, maxLength) => {
  if (typeof value !== "string") return "";
  return value.trim().slice(0, maxLength);
}, "sanitizeValue");
var parseAndValidateContactRequest = /* @__PURE__ */ __name2((input) => {
  ...
  const data = {
    ...
    message: sanitizeValue(record.message, 5e3),
    turnstileToken: sanitizeValue(record.turnstileToken, 4e3)
  };
  const fields = {};
...
var jsonResponse = /* @__PURE__ */ __name2((status, payload) => new Response(JSON.stringify(payload), {
  status,
  headers: {
...
var getClientIp = /* @__PURE__ */ __name2((request) => request.headers.get("CF-Connecting-IP") || "unknown", "getClientIp");
var hashRateLimitKey = /* @__PURE__ */ __name2(async (value) => {
  const encoded = new TextEncoder().encode(value);
  const digest = await crypto.subtle.digest("SHA-256", encoded);
  return [...new Uint8Array(digest)].map((byte) => byte.toString(16).padStart(2, "0")).join("");
}, "hashRateLimitKey");
var enforceRateLimit = /* @__PURE__ */ __name2(async (request, env) => {
  ...
  if (env.CONTACT_RATE_LIMIT_KV) {
    const currentEntry2 = await env.CONTACT_RATE_LIMIT_KV.get(key, { type: "json" });
    const initialState2 = { count: 0, resetAt: now + windowSeconds * 1e3 };
    const state2 = currentEntry2 && currentEntry2.resetAt > now ? currentEntry2 : initialState2;
    if (state2.count >= maxRequests) {
      return {
        allowed: false,
        retryAfterSeconds: Math.max(1, Math.ceil((state2.resetAt - now) / 1e3))
      };
    }
    state2.count += 1;
    await env.CONTACT_RATE_LIMIT_KV.put(key, JSON.stringify(state2), {
      expirationTtl: windowSeconds
    });
    return { allowed: true };
  }
  const currentEntry = inMemoryRateLimitState.get(key);
  const initialState = { count: 0, resetAt: now + windowSeconds * 1e3 };
  const state = currentEntry && currentEntry.resetAt > now ? currentEntry : initialState;
  if (state.count >= maxRequests) {
    return {
      allowed: false,
      retryAfterSeconds: Math.max(1, Math.ceil((state.resetAt - now) / 1e3))
    };
  }
  state.count += 1;
...
var requiredEnvVars = ["TURNSTILE_SECRET_KEY", "CONTACT_TO_EMAIL", "RESEND_API_KEY"];
...
// .wrangler/tmp/bundle-Ehn9Oz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Ehn9Oz/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.31663766019398165.js.map
